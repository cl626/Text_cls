...
# 调用词向量化模型
path = "google-bert/bert-base-uncased"  # "bert-base-uncased"
...
train_union = tokenizer(process_imdb.train_samples,max_length=MAXLEN,padding='max_length',truncation=True,return_tensors='pt')

# 单词通过Berttokenizer转为词向量后，和标签一起传递给张量数据集TensorDataset，构建数据集，简化传统的Dataset的操作
train_set = TensorDataset(train_union['input_ids'], train_union['attention_mask'],
...
test_union = tokenizer(process_imdb.test_samples,max_length=MAXLEN,padding='max_length',truncation=True,return_tensors='pt')
test_set = TensorDataset(test_union['input_ids'],test_union['attention_mask'],
